...
int main(int argc, char** argv) 
{
    ...
    mqd_t q; // descriptors
	struct mq_attr attrq; // attributes
	attr.mq_maxmsg = 10; // capacity
	attr.mq_msgsize = MAXLENGTH; // max msg size in bytes
    
    char q_name[MAXLENGTH];
    snprintf(q_name, MAXLENGTH, "/q%s", message+9);
    
    if((q=TEMP_FAILURE_RETRY(mq_open(q_name, O_RDWR, 0600, &attrq)))==(mqd_t)-1) ERR("mq open q");
    printf("prog2 with pid %d: message queue with name: %s opened\n", getpid(), q_name);

	char buffer[MAXLENGTH];
    mq_receive(q, buffer, MAXLENGTH ,NULL);
	buffer[sizeof(buffer)-1]='\0';
    printf("Prog2 received message: %s\n", buffer);
